...
## Custom provider: Cursor CLI

The Cursor CLI provider is a native Cursor Agent API integration (`agent.v1.AgentService/Run`), separate from the existing Cursor IDE provider (`cursor`/`cu`, which uses `ChatService/StreamUnifiedChatWithTools`). It uses OMP-derived generated protobuf bindings and OAuth PKCE polling.

### User-facing contract

- Provider ID and alias: `cursor-cli`
- Credential: Cursor OAuth access token obtained through PKCE browser login + polling
- Auth flow: opens `cursor.com/loginDeepControl`, polls `api2.cursor.sh/auth/poll`
- Endpoint: `https://api2.cursor.sh/agent.v1.AgentService/Run`
- Canonical models: `auto`, `composer-2.5`, `default`
- Capabilities: reasoning, tools; 200K context window
- Live model discovery is intentionally deferred.

### Ownership map

| Concern | Files | Rule |
|---|---|---|
| Provider metadata | `open-sse/providers/registry/cursor-cli.js` | Registry is source of truth for identity, OAuth config, models. |
| Registry aggregation | `open-sse/providers/registry/index.js` | Auto-generated; re-add `cursor-cli` import after regenerating. |
| Model capabilities | `open-sse/providers/capabilities.js` | Keep composer-2.5/composer-2 capability entries. |
| Executor registration | `open-sse/executors/index.js` | `CursorCliExecutor` must be imported, instantiated, and re-exported. |
| Executor entry point | `open-sse/executors/cursor-cli.js` | Owns HTTP/2 streaming, AgentRunRequest, SSE transform, one credential only. |
| Protocol/framing | `open-sse/executors/cursor-cli/protocol.js` | Connect framing, headers, trailer parsing, heartbeat constants. |
| Generated bindings | `open-sse/executors/cursor-cli/proto/agent_pb.js` | Generated code from OMP `agent.proto`. Do not hand-edit. Replace atomically from verified source. |
| Error classification | `open-sse/executors/cursor-cli/errors.js` | Shared classifier and credential sanitization. |
| Session manager | `open-sse/executors/cursor-cli/session.js` | Bounded, TTL-evicted, for inline tool-result continuation only. |
| Image safety | `open-sse/executors/cursor-cli/images.js` | Data URI decode, size/count caps. |
| OAuth provider | `src/lib/oauth/providers.js` | `cursor-cli` entry in PROVIDERS, builds login URL, polls for token. |
| OAuth constants | `src/lib/oauth/constants/oauth.js` | `CURSOR_CLI_CONFIG` from registry. |
| OAuth service | `src/lib/oauth/services/cursor-cli.js` | CLI/script-driven OAuth helper (for non-dashboard flows). |
| OAuth service index | `src/lib/oauth/services/index.js` | Keep `CursorCliService` export. |
| Tests | `tests/unit/cursor-cli-protocol.test.js`, `tests/unit/cursor-cli-session.test.js` | All credentials and I/O must be fake/mocked. |
| Specification | `.kiro/specs/cursor-cli/` | Requirements, design, and task history. Update with design changes. |

### Key merge/conflict rules

- `open-sse/providers/registry/index.js` is auto-generated. After upstream merge, re-add the `cursor-cli` import and re-run the registry generator.
- `open-sse/executors/index.js` — keep `CursorCliExecutor` import, instantiation, map entry, and re-export alongside upstream additions.
- `src/lib/oauth/providers.js` — keep the `cursor-cli` PROVIDERS entry and the `CURSOR_CLI_CONFIG` import.
- `src/lib/oauth/constants/oauth.js` — keep `CURSOR_CLI_CONFIG` and the `CURSOR_CLI` PROVIDERS enum entry.
- `src/lib/oauth/services/index.js` — keep `CursorCliService` export.
- `open-sse/providers/capabilities.js` — keep cursor-cli composer model entries.
- All new files under `open-sse/executors/cursor-cli/` are custom and must be retained.
- `tests/unit/cursor-cli-*.test.js` — retain all custom test files.
- The `cursor` / `cu` IDE provider must remain untouched; upstream changes to it are safe to accept.

### Generated protobuf update procedure

The current generated binding source is the verified OMP `agent.proto` + `agent_pb.ts`. If Cursor changes its agent protocol:

1. Obtain the updated generated binding from a verified OMP reference.
2. Replace `open-sse/executors/cursor-cli/proto/agent_pb.js` atomically.
3. Verify `AgentClientMessageSchema`, `AgentRunRequestSchema`, `AgentServerMessageSchema` exports are intact.
4. Run focused cursor-cli tests and the production build before merging.
